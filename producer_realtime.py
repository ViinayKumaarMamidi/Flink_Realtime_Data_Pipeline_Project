...
# Sample Pickup Locations (mostly in Manhattan, plus the two airports): 
...
# For simplicity, we use the same pool of locations for dropoffs.
# In reality, dropoff locations are more spread out across the city, but this is sufficient for our demo.

DROPOFF_LOCATIONS = PICKUP_LOCATIONS  # same pool for simplicity


# Simulate a stream of taxi rides with some late events (3-10 seconds old).
...
# Rides are serialized as JSON before being sent to the Kafka broker: 

def ride_serializer(ride):
    return json.dumps(dataclasses.asdict(ride)).encode('utf-8')

# Source Configuratuion for Kafka 
...
# Assign topics and send events in an infinite loop until interrupted by the user (Ctrl+C).
